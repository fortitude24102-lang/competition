...
try {
    ...
    & $RiscvGcc -std=gnu11 -Os -Wall -Wextra -Werror -march=rv32im_zicsr -mabi=ilp32 -ffreestanding -ffunction-sections -fdata-sections -Isw/efinix_gpu/include -isystem "$bsp/include" -isystem "$soc/software/standalone/driver" -DUSE_GP -DNO_LIBC_INIT_ARRAY -nostartfiles "-T$bsp/linker/default.ld" '-Tsw/efinix_gpu/linker.ld' '-Wl,--gc-sections' "-Wl,-Map,$out/gpu_demo.map" "$soc/software/standalone/common/start.S" sw/efinix_gpu/src/main.c sw/efinix_gpu/src/gpu.c sw/efinix_gpu/src/benchmark.c sw/efinix_gpu/src/golden_renderer.c sw/efinix_gpu/src/rgb565.c sw/efinix_gpu/src/assets.c sw/efinix_gpu/src/framebuffer.c sw/efinix_gpu/src/hud.c sw/efinix_gpu/src/game.c -o "$out/gpu_demo.elf"
