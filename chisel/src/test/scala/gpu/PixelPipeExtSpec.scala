...
class PixelPipeExtSpec extends AnyFunSpec with StableChiselSim with Matchers {
  it("runs real Copy and Fill RTL and preserves pending pixels across stalls") {
    simulate(new PixelPipeHarness) { dut =>
      ...
      dut.clock.step()
      val beats = Seq((2, 0xf800, 0x001f, 0xf800, true),
        (1, 0xf800, 0x07e0, 0x07e0, true), (4, 0xffff, 0xffff, 0, false))
      for ((op, fg, fill, pixel, write) <- beats) {
